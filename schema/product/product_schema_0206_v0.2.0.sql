...
CREATE TABLE `autonomous_options` (
    ...
    `option_name`                   VARCHAR(100)    NOT NULL    UNIQUE                      COMMENT '옵션명',               
...
--
-- autonomous_options_combinations
--
CREATE TABLE `autonomous_options_combinations` (
    ...
    `stack_volume`                  INT             NULL                                    COMMENT '재고수량',
    `stack_price`                   INT             NOT NULL                                COMMENT '추가금액',
    `is_used`                       TINYINT         NOT NULL                                COMMENT '사용여부',
    `created_at`                    DATETIME        NOT NULL    DEFAULT CURRENT_TIMESTAMP   COMMENT '생성시간',
    `updated_at`                    DATETIME        NULL                                    COMMENT '수정시간',
    `creator_id`                    BIGINT          NULL                                    COMMENT '생성자',
    ...
    CONSTRAINT autonomous_options_combinations_options_types_id_fkey FOREIGN KEY (autonomous_options_types_id) REFERENCES autonomous_options_types(id)
...
CREATE TABLE `products` (
    ...
    `detailed_info`                 LONGTEXT        NULL                                    COMMENT '상세상품정보',
...
INSERT INTO products (
    ...
    TRUE, -- is_displayed
    TRUE, -- is_used
    (SELECT id from first_categories WHERE name='아우터' LIMIT 1), -- first_categories_id
    (SELECT id from second_categories WHERE name='코트' LIMIT 1), -- second_categories_id
    FALSE, -- information_notice_use
    '한줄설명_1', -- one_line_description
    'image_url', -- main_image
    (SELECT id from color_filters WHERE kr_name='빨강' LIMIT 1), -- color_filters_id
    (SELECT id from style_filters WHERE name='심플베이직' LIMIT 1), -- style_filters_id
    FALSE, -- is_age_filter_input
    (SELECT id from option_info WHERE id=1 LIMIT 1), -- option_info_id
    10000, -- price
